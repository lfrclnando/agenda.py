...
# See favorites
def see_favorites(agenda):
    favorites = [info for contact, info in agenda if info["favorito"] == "Sim"]
    if favorites:
        for indice, info in enumerate(favorites, start=1):
            print(f"{indice}. {info['nome']} - {info['telefone']} - {info['e-mail']} - {info['favorito']}")
    else:
